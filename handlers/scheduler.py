...
# Обработчик для перепланирования поста
@router.message(StateFilter(PostCreation.scheduling))
async def handle_rescheduling_input(message: Message, state: FSMContext):
    """Обработка ввода при перепланировании поста"""
    try:
        data = await state.get_data()
        ...
        if rescheduling_post_id:
            ...
            # Это обычное планирование нового поста
            await handle_custom_time_input(message, state)

    except Exception as e:
        logger.error(f"Ошибка обработки ввода времени: {e}")
...
async def handle_auto_distribute(callback: CallbackQuery, post_id: int, state: FSMContext):
    ...
    try:
        ...
        logger.error(f"Ошибка обработки пользовательского времени: {e}")
        await callback.answer("❌ Произошла ошибка при планировании")
        await state.set_state(Menu.main)